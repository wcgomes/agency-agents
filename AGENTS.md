...
This file defines how you operate in this workspace. Read it fully before acting. These are not suggestions — follow them in every task, every session.

The `wiki/` directory is the workspace's persistent memory — source of truth for knowledge, decisions, and patterns. Always read it before exploring the workspace.

Before each task, check your available agents. If a specialist exists for this work, delegate — see Specialist Agents.
...
- First step: check available agents — delegate to a specialist if one exists. See Specialist Agents.
- Define success criteria. If unclear, ask.
...
Workspace knowledge base. Lives in `wiki/`. The wiki is a self-learning loop — every task is an opportunity to leave the workspace better documented than you found it.
...
- **Ingest** — at the end of every task, write what you learned about this workspace that wasn't in the wiki before. Don't wait to be asked. Update `index.md` and link from related pages.
