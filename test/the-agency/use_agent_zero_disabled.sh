...
check "on-create creates AGENTS.md with agency guidance only (when disabled)" bash -c '
    ...
    # Run the script - since the marker file does not exist, it should add agency guidance only
...
check "on-create updates existing AGENTS.md with agency guidance only (when disabled)" bash -c '
    tmp_ws="$(mktemp -d)"
...
EOF
    ...
    # Run the script - since the marker file does not exist, it should update with agency guidance only
    /usr/local/share/devcontainer-features/the-agency-on-create.sh
